...
task("initialize", "Initialize the station by registering the main upkeep")
    ...
    .addOptionalParam(
        "amount",
        "The LINK token amount to fund the station upkeep",
        "10000000000000000000",
    )
    .addOptionalParam(
        "gasLimit",
        "The maximum gas limit that will be used for txns by the station upkeep",
    )
    .setAction(async (taskArgs, { ethers }) => {
        ...
        if (taskArgs.registry == ethers.ZeroAddress) {
            console.log(
                "Chainlink Automation forwarder not set, setForwarder task must be executed",
            )
        }
    })
